        ...
        this.certificateService.getCertificateByCode(codigo).subscribe({
            next: (response) => {
                if (response.success && response.data) {
                    const htmlContent = this.generateFullHtml(response.data);
                    this.openHtmlInNewTab(htmlContent, codigo);
                } else {
            ...
            error: (error) => {
                ...
                alert(`Error: ${error.message || 'No se pudo cargar el certificado'}`);
            }
        });
    }

    private generateFullHtml(certificado: CertificateDetail): string {
...
<head>
    ...
    <title>Certificado de Inspección Sanitaria</title>
    <style>
        body {
        ...
        .table-borderless[cellspacing="0"] {
            border-top: 1px solid #ddd;
        ...
        .table-borderless[cellspacing="0"] th,
        .table-borderless[cellspacing="0"] td {
...
<body>
    <div class="container" style="width: 950px;">
        ...
        ${this.generateCertificateContent(certificado)}
    </div>
</body>
</html>`;
    }

    private generateCertificateContent(certificado: CertificateDetail): string {
        if (!certificado) {
